...
private val zhMap = mapOf(
    ...
    "Image attachment" to "图片附件",
    "Pick image" to "从相册选择图片",
    "Pick image from files" to "从文件选择图片",
    "Take photo" to "拍照",
    "All" to "全部",
    "CPU" to "CPU",
    "NPU" to "NPU",
    ...
    ,"Welcome to Intelligence Lab" to "欢迎使用 Intelligence Lab"
    ...
    ,"Please read carefully before using Intelligence Lab" to "使用 Intelligence Lab 前请仔细阅读"
    ,"I Accept" to "我同意"
    ...
    ,"Bypass Model Check" to "绕过模型检查"
    ,"Force tool calling on models without a chat template. May cause errors or unexpected output." to "对没有聊天模板的模型强制启用工具调用，可能导致错误或非预期输出。"
    ...
    ,"RAG Manager" to "知识库管理"
    ,"Import Neuron Package" to "导入知识库包"
    ,"RAG Management" to "知识库管理"
    ,"Loaded" to "已加载"
    ,"Ready" to "就绪"
    ,"Back to repos" to "返回仓库列表"
    ,"Device Information" to "设备信息"
    ,"Model Repositories" to "模型仓库"
    ,"HuggingFace GGUF Explorer" to "HuggingFace GGUF 浏览器"
    ,"Searching HuggingFace..." to "正在搜索 HuggingFace..."
    ,"Gated" to "受限"
    ,"Added" to "已添加"
    ,"Add repository" to "添加仓库"
    ,"Cancel Download" to "取消下载"
    ,"No RAGs loaded" to "未加载知识库"
    ,"No RAGs installed" to "未安装知识库"
    ,"Load a RAG from the Installed tab to enable context-aware responses" to "请在“已安装”页加载知识库，以启用上下文增强回复"
    ,"Create a new RAG or install one from a file" to "创建新知识库，或从文件安装知识库"
    ,"Install a RAG package or create one from the Create tab" to "请安装知识库包，或在“创建”页新建知识库"
    ,"Load a RAG from the Installed tab to use it in chats" to "请在“已安装”页加载知识库后再在聊天中使用"
    ,"RAG file path not found" to "未找到知识库文件路径"
    ,"RAG file not found" to "未找到知识库文件"
    ,"Create RAG" to "创建知识库"
    ,"Enter name & add content" to "请输入名称并添加内容"
    ,"Enter a name" to "请输入名称"
    ,"Embedding not ready" to "向量模型尚未就绪"
    ,"Add content or pick a file" to "请添加内容或选择文件"
    ,"Fill required fields" to "请填写必填项"
    ,"Advanced Options" to "高级选项"
    ,"Embedding Model" to "向量模型"
    ,"Downloaded — tap Initialize" to "已下载，点击初始化"
    ,"Required for RAG features" to "知识库功能必需"
    ,"Encryption" to "加密"
    ,"Password-protect this RAG" to "为该知识库启用密码保护"
    ,"Admin Password" to "管理员密码"
    ,"Embedded" to "嵌入式"
    ,"Transient" to "临时加载"
    ,"Add a user who can access this RAG" to "添加可访问该知识库的用户"
    ,"This RAG is encrypted. Enter the password to load it." to "该知识库已加密，请输入密码后加载。"
    ,"Nodes" to "节点数"
    ,"Embedding Dimension" to "向量维度"
    ,"Language" to "语言"
    ,"Created" to "创建时间"
    ,"Last Loaded" to "最后加载"
    ,"Statistics" to "统计信息"
